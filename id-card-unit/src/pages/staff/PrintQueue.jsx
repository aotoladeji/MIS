...
export default function PrintQueue() {
  ...
  const syncCards = async (silent = false) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/print-queue/sync', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await response.json();
  ...
  const clearQueue = async () => {
    ...
    setSyncing(true);
    try {
      const response = await fetch('http://localhost:5000/api/print-queue/clear', {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await response.json();
  ...
  const printCardDirect = async (card) => {
    try {
      console.log('Fetching card image for card ID:', card.card_id);
      
      // Fetch the generated card image through our backend proxy (avoids CORS)
      const response = await fetch(`http://localhost:5000/api/card-images/${card.card_id}/image`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      console.log('Card image fetched successfully, size:', blob.size, 'bytes, type:', blob.type);
      ...
      const markResponse = await fetch(`http://localhost:5000/api/print-queue/${card.id}/printed`, {
        method: 'POST',
        headers: {
  ...
  const markAsFailed = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${id}/failed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const deleteCard = async (id, name) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const retryPrint = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${id}/retry`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
