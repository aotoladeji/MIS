...
export default function PrintCardModal({ card, onClose, onPrintComplete }) {
  ...
  const generatePDF = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${card.id}/generate-pdf`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const sendToDirectPrinter = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${card.id}/print-direct`, {
        method: 'POST',
        headers: {
  ...
  const markAsPrinted = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/print-queue/${card.id}/printed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
